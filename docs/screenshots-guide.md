...
## Task 2 — Secure CI/CD & Supply Chain *(pending)*

- Actions run summary: all gates green
- A build **failing** on a planted secret / critical CVE — the gate *blocking* is the evidence
- `cosign verify` output showing the workflow identity
- GitHub Security tab with SARIF results
- ArgoCD UI: Synced/Healthy, then drift after a manual `kubectl edit`, then self-heal
